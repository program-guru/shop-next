...
const CATEGORIES = ["Running", "Casual", "Basketball", "Training", "Skateboarding"];
...
export default function Sidebar() {
  ...
  function toggleSelection(item: string, current: string[], setFn: (val: string[]) => void) {
  ...
  const sortedBrands = useMemo(() => sortChips(BRANDS, selectedBrands), [selectedBrands]);
  const sortedCategories = useMemo(() => sortChips(CATEGORIES, selectedCategories), [selectedCategories]);
    ...
    <>
      ...
      >
        ...
        ">
          ...
          {/* 2. Brands */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-text uppercase tracking-wider">Brands</label>
          ...
          {/* 3. Categories */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-text uppercase tracking-wider">Category</label>
            <div className="flex flex-wrap gap-1.5">
          ...
          <div className="space-y-3">
            <label className="text-xs font-semibold text-text uppercase tracking-wider">Price Range</label>
            ...
            <div className="relative w-full h-6 flex items-center select-none touch-none">
                ...
                <div 
                    ...
                    style={{ left: `calc(${(minPrice / MAX_PRICE_LIMIT) * 100}% - 7px)` }}
                ></div>
                <div 
                    className="absolute w-3.5 h-3.5 bg-white border-[1.5px] border-primary rounded-full shadow z-10 pointer-events-none"
                    style={{ left: `calc(${(maxPrice / MAX_PRICE_LIMIT) * 100}% - 7px)` }}
                ></div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-baseline">
                <label className="text-xs font-semibold text-text uppercase tracking-wider">Rating</label>
                {minRating && <span className="text-[10px] text-text-muted">{minRating}+ Stars</span>}
            </div>
            
            <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => {
                    const isFilled = (hoverRating || minRating || 0) >= star;
                    return (
                        <button
                            key={star}
                        ...
                        >
                            <Star 
                                className={`w-6 h-6 transition-colors duration-200 ${
                                  isFilled ? "text-yellow-400" : "text-border"
                                }`} 
          ...
          {/* 6. Sizes */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-text uppercase tracking-wider">Sizes</label>
          ...
          <div className="space-y-2 pb-20 md:pb-0">
            <label className="text-xs font-semibold text-text uppercase tracking-wider">Sort By</label>
            <div className="relative">
              <select 
                value={sortBy || ""} 
